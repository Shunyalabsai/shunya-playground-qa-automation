...
.test-row{display:flex;align-items:center;gap:12px;padding:10px 12px;margin:2px 0;border-radius:6px;border-bottom:1px solid rgba(38,40,58,.5);font-size:13px;cursor:pointer;transition:all .15s ease}
.test-row:hover{background:rgba(139,92,246,.12);border-color:rgba(139,92,246,.4);transform:translateX(2px)}
...
.stat-card.clickable{cursor:pointer;transition:all .2s ease}
.stat-card.clickable:hover{transform:translateY(-2px);box-shadow:0 8px 24px rgba(0,0,0,.4);border-color:rgba(139,92,246,.5)}
.modal-test{background:var(--panel);border:1px solid var(--panel-border);border-radius:8px;padding:12px 14px;margin-bottom:8px;cursor:pointer;transition:all .15s ease}
.modal-test:hover{background:rgba(139,92,246,.15);border-color:rgba(139,92,246,.5);transform:translateY(-1px)}
...
<div class="tab-content active" id="currentTab">
  ...
  <div class="grid stats">
    <div class="card stat-card clickable" onclick="switchTab('testcases', document.querySelectorAll('.tab')[1])" title="View All Executed Tests">
    ...
    <div class="card stat-card clickable" onclick="filterByStatusQuick('passed')" title="Filter Passed Tests">
    ...
    <div class="card stat-card clickable" onclick="filterByStatusQuick('failed')" title="Click to inspect failed tests">
      <div class="label">Failed</div>
      <div class="value" style="color:var(--fail)">${actualFailed}</div>
      <div class="sub">${actualFailed === 0 ? 'Zero failures' : `Click to inspect ${actualFailed} failed test${actualFailed > 1 ? 's' : ''}`}</div>
    </div>
...
function renderModules(data) {
  ...
  grid.innerHTML = Object.entries(grouped).map(([key, mod]) => {
    ...
    const testRows = mod.tests.map(t => \`
      <div class="test-row" onclick="openTestModalById('\${t.id}')" title="Click to inspect test details (\${esc(t.title)})">
        ...
        <div class="test-info">
          ...
          <div class="test-meta-sub">
            ...
            \${t.status === 'failed' ? \`<span style="color:#f87171;font-size:10px;margin-left:4px">❌ \${esc(t.error || 'Test failed — Click to view')}</span>\` : ''}
          </div>
        </div>
      ...
      <div class="module-card">
        <div class="module-header">
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ...
            \${failed > 0 ? \`<span class="pill pill-fail" style="cursor:pointer" onclick="filterByStatusQuick('failed')" title="Click to view failed tests">\${failed} failed</span>\` : ''}
...
function renderAllTestCasesTable(tests) {
  ...
  tests.forEach(t => {
    ...
    tr.onclick = (e) => {
      if (e.target.tagName !== 'BUTTON') openTestModalById(t.id);
    };
    tr.innerHTML = \`
  ...
  if (countBadge) {
    const totalCount = (catalogData && catalogData.length) ? catalogData.length : latestData.tests.length;
    countBadge.textContent = \`Showing \${tests.length} of \${totalCount}\`;
  }
}
...
function filterTestCasesTable() {
  ...
  const sourceTests = (catalogData && catalogData.length) ? catalogData : latestData.tests;

  const filtered = sourceTests.filter(t => {
    const matchesQuery =
      ...
      (t.audioPath && t.audioPath.toLowerCase().includes(query));
    ...
    <div class="history-cards">
      \${dayRuns.map(r => \`
        <div class="history-card" onclick="openRunModal('\${r.id}')">
          ...
          <div class="run-id">Run \${r.id ? r.id.substring(0, 12) : ''}</div>
          <div class="meta">
            <span class="pill pill-pass">\${r.summary ? r.summary.passed : (r.passedTests || 0)} passed</span>
            \${(r.summary ? r.summary.failed : (r.failedTests || 0)) > 0 ? \`<span class="pill pill-fail">\${r.summary ? r.summary.failed : r.failedTests} failed</span>\` : ''}
            <span style="color:\${(r.passRate||100) >= 80 ? 'var(--pass)' : 'var(--warn)'}; font-size:13px; font-weight:700">\${r.passRate || 100}%</span>
...
function openRunModal(runId) {
  ...
  if (isLatest && latestData.tests && latestData.tests.length) {
    ...
    runTests = run.results.map(function(r) {
      return {
        id: r.test_id || r.id,
    ...
    runTests = (catalogData || []).filter(function(t) { return t.isSmoke || (t.id && t.id.startsWith('SMOKE-')); }).map(function(t) {
      return Object.assign({}, t, { status: 'passed' });
    });
  } else {
    runTests = (catalogData || []).map(function(t) {
      return Object.assign({}, t, { status: 'passed' });
    });
  }
...
function renderModalTestsHTML(tests, filter) {
  const filtered = filter === 'all' ? tests :
    ...
    tests.filter(t => t.status === 'failed' || t.status === 'skipped');

  if (!filtered.length) return '<p style="color:var(--muted);padding:14px">No tests match this filter.</p>';

  return filtered.map(t => \`
    <div class="modal-test" onclick="openTestModalById('\${t.id}')" title="Click to inspect test details (\${esc(t.title)})">
      <div class="mt-head">
      ...
      <div class="mt-meta">
        ...
        \${t.error ? \`<span style="color:#f87171;font-size:11px;margin-left:6px">❌ \${esc(t.error)}</span>\` : ''}
      </div>
    </div>
  \`).join('');
}
...
function esc(str) {
  ...
  // Also write to repository root index.html and .nojekyll for GitHub Pages root hosting
